...
new6(A,B,C,D,E) :- B-rat(1,2)*D=<rat(-1,2), new8(A,B,C,D,E).
new5(A,B,C,D,E) :- F=:=rat(5,1), G=:=rat(-10,1), A=:=rat(4,1), new4(F,B,C,G,E).
new5(A,B,C,D,E) :- F=:=rat(1,1)+A, A=<rat(3,1), new4(F,B,C,D,E).
new5(A,B,C,D,E) :- F=:=rat(1,1)+A, A>=rat(5,1), new4(F,B,C,D,E).
new4(A,B,C,D,E) :- F=:=rat(2,1)+D, A-B=<rat(0,1), new5(A,B,C,F,E).
new4(A,B,C,D,E) :- A-B>=rat(1,1), new6(A,B,C,D,E).
new3(A,B,C,D,E) :- F=:=rat(1,1), G=:=H, I=:=rat(0,1), H>=rat(0,1), 
          new4(F,H,G,I,E).
new2 :- new3(A,B,C,D,E).
new1 :- new2.
inv1 :- \+new1.
